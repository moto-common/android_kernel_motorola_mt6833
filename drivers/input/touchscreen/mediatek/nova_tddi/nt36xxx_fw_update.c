...
const struct firmware *fw_entry_n = NULL;
...
static int32_t nvt_get_fw_need_write_size(const struct firmware *fw_entry_n)
{
	int32_t i = 0;
	int32_t total_sectors_to_check = 0;

	total_sectors_to_check = fw_entry_n->size / FLASH_SECTOR_SIZE;
	...
	for (i = total_sectors_to_check; i > 0; i--) {
		...
		if (strncmp(&fw_entry_n->data[i * FLASH_SECTOR_SIZE - NVT_FLASH_END_FLAG_LEN], "NVT", NVT_FLASH_END_FLAG_LEN) == 0) {
		...
		if (strncmp(&fw_entry_n->data[i * FLASH_SECTOR_SIZE - NVT_FLASH_END_FLAG_LEN], "MOD", NVT_FLASH_END_FLAG_LEN) == 0) {
			fw_need_write_size = i * FLASH_SECTOR_SIZE;
...
static int32_t update_firmware_request(char *filename)
{
	...
	while (1) {
		...
		ret = request_firmware(&fw_entry_n, filename, &ts->client->dev);
		if (ret) {
		...
		if (nvt_get_fw_need_write_size(fw_entry_n)) {
		...
		if (*(fw_entry_n->data + FW_BIN_VER_OFFSET) + *(fw_entry_n->data + FW_BIN_VER_BAR_OFFSET) != 0xFF) {
			NVT_ERR("bin file FW_VER + FW_VER_BAR should be 0xFF!\n");
			NVT_ERR("FW_VER=0x%02X, FW_VER_BAR=0x%02X\n", *(fw_entry_n->data+FW_BIN_VER_OFFSET), *(fw_entry_n->data+FW_BIN_VER_BAR_OFFSET));
			ret = -ENOEXEC;
			goto invalid;
		}

		/* BIN Header Parser */
		ret = nvt_bin_header_parser(fw_entry_n->data, fw_entry_n->size);
		if (ret) {
...
static int32_t nvt_download_firmware_hw_crc(void)
{
	...
	while (1) {
		...
		ret = nvt_write_firmware(fw_entry_n->data, fw_entry_n->size);
		if (ret) {
